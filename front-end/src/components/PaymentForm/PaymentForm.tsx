...
const CHECKOUT_MUTATION = gql`
  mutation Checkout($paymentMethod: String!, $address: String!) {
...
const PaymentForm = () => {
  ...
  const [fazerCheckout, { loading }] = useMutation(CHECKOUT_MUTATION);
  ...
  const onSubmit = async (data: PaymentFormData) => {
    const enderecoCompleto = `${data.endereco}, ${data.numeroComplemento}`;
    
    try {
      const { data: responseData } = await fazerCheckout({
        variables: {
      ...
      if (responseData?.checkout?.orderId) {
        alert("Pedido realizado com sucesso! Redirecionando para o resumo.");
        navigate("/resumo-pedido", { state: { orderId: responseData.checkout.orderId } });
      }
    } catch (error) {
      
      if (error instanceof Error) {
        alert(`Erro ao finalizar o pedido: ${error.message}`);
        console.error("Erro de checkout GraphQL:", error);
      } else {
        alert("Ocorreu um erro desconhecido ao finalizar o pedido.");
